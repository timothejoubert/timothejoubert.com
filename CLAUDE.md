...
- ⚠️ Never call `writeClient.migrate()` exploratorily (e.g. one-off debug scripts) against this real Prismic repo. A repo's pending migration release is a single shared, persistent object across every call — a partial/test `data` object passed to `migration.updateDocument()` gets queued into that same release, and a later unrelated `--publish` run (yours or the real import script's) can publish those stale test writes, silently wiping real published fields back to `null`. Debug against a copy of the payload/logic instead of a live `migrate()` call, or immediately re-run a full `pnpm projects:import-source <uid>` afterward to overwrite any test pollution.
- `pageSize` on the Asset API (`https://asset-api.prismic.io/assets`) is not respected — every page always returns 10 items regardless of the requested value; paginate via the `cursor` field in the response instead. The API also rate-limits (`429 Too Many Requests`) fairly aggressively — batch script calls to it (dedup lookups, bulk listing/deleting) need retry-with-backoff and a small delay between requests.
